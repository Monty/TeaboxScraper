...
/<h5 class="steeping-note-hot">/ {
    ...
    if (note ~ /185/) note = "185-194F"

    if (note ~ /176/) note = "176-185F"
...
/<div class="condiment-cnt"/, /<\/div>/ {
    if ($0 ~ /<p>/) {
        ...
        condiment = fld[3]
        condimentsFound += 1
        condimentsFound == 1 ? drink_with = condiment : drink_with = drink_with\
            ", "\
            condiment
...
/<h3 class="header-1"/ {
    ...
    gsub(\
        /<p style="text-align: left;"><span style="font-size: small; font-family: arial, helvetica, sans-serif;">/,
        ""\
    )
    gsub(/<\/span>/, " ")
    sub(/ $/, "")
